...
/*
 * port/caliptra/caliptra_hwmodel.c
 *
 * Caliptra hw-model mailbox transport for wolfHSM integration testing.
 *
 * Implements caliptra_mailbox_exec() — the integrator-supplied transport hook
 * declared in wolfssl's caliptra_port.h — using the Caliptra hw-model C
 * binding.
 *
...
/* wolfSSL word32 type — caliptra_mailbox_exec must match caliptra_port.h */
...
static void mbox_write(uint32_t reg_offset, uint32_t val)
{
...
static uint32_t mbox_read(uint32_t reg_offset)
{
    uint32_t val = 0;
    uint32_t addr = EXTERNAL_PERIPH_BASE
                  + CALIPTRA_TOP_REG_MBOX_CSR_BASE_ADDR
...
static void soc_write(uint32_t reg_offset, uint32_t val)
{
    uint32_t addr = EXTERNAL_PERIPH_BASE
...
static void soc_write_direct(uint32_t periph_offset, uint32_t val)
{
...
{
    int i;
    for (i = 0; i < MBOX_LOCK_MAX_STEPS; i++) {
...
/* =========================================================================
 * Mailbox FIFO write
 * Writes req_len bytes from req into the DATAIN register word by word.
...
static void mbox_write_fifo(const void *req, word32 req_len)
{
    ...
    while (remaining >= 4) {
        uint32_t w;
        memcpy(&w, p, 4);
        mbox_write(MBOX_CSR_MBOX_DATAIN, w);
        p += 4;
        remaining -= 4;
    }
    if (remaining > 0) {
        uint32_t w = 0;
        memcpy(&w, p, remaining);
        mbox_write(MBOX_CSR_MBOX_DATAIN, w);
    }
}
 ...
 * hw_dlen is the byte count reported by the hardware (MBOX_DLEN after execute).
 * ========================================================================= */
...
static void mbox_read_fifo(void *resp, word32 resp_len, uint32_t hw_dlen)
{
    ...
    for (i = 0; i < full_words; i++) {
        uint32_t w = mbox_read(MBOX_CSR_MBOX_DATAOUT);
        memcpy(p, &w, 4);
        p += 4;
    }
    if (tail > 0) {
...
static int load_file(const char *path, struct caliptra_buffer *out)
{
    ...
    sz = ftell(fp);
    if (sz <= 0) {
        fprintf(stderr, "caliptra_hwmodel: '%s' empty or ftell failed\n", path);
        fclose(fp);
        return -EIO;
    }
    ...
    if (got != (size_t)sz) {
        fprintf(stderr,
                "caliptra_hwmodel: short read on '%s': %zu of %ld bytes\n",
                path, got, sz);
        free(buf);
        return -EIO;
    }
...
int caliptra_hwmodel_init(const char *rom_path, const char *fw_path)
{
    ...
    /* Zero-init first so optional_obf_key and any future fields default to
     * {NULL, 0}.  security_state = DBG_UNLOCKED_UNPROVISIONED allows booting
     * with zero fuses in development/simulation environments. */
    memset(&params, 0, sizeof(params));
    params.rom            = rom;
    params.dccm.data      = &empty_byte;
    params.dccm.len       = 0;
    params.iccm.data      = &empty_byte;
    params.iccm.len       = 0;
    params.security_state = CALIPTRA_SEC_STATE_DBG_UNLOCKED_UNPROVISIONED;
    params.soc_user       = 0;

    init_rc = caliptra_model_init_default(params, &g_model);
 ...
 *
 * Returns 0 on success.  Returns non-zero if the hardware rejected the
 * command (CMD_FAILURE), the mailbox lock could not be acquired, or the
